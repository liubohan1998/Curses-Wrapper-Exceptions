...
TEST_F(CursesWrapperTest, CursorMoveExceptionTest) {
  const int numRows = 10;
  const int numCols = 10;
  const int rowStart = 3;
  const int colStart = 7;
  ...
  EXPECT_THROW(win.moveCursor(-1, 3), std::out_of_range);
  EXPECT_THROW(win.moveCursor(4, -2), std::out_of_range);
  EXPECT_THROW(win.moveCursor(10, 3), std::out_of_range);
  EXPECT_THROW(win.moveCursor(4, 10), std::out_of_range);
  EXPECT_THROW(win.moveCursor(10, 10), std::out_of_range);
  EXPECT_THROW(win.moveCursor(2, 13), std::out_of_range);
  EXPECT_THROW(win.moveCursor(5, 30), std::out_of_range);
}

TEST_F(CursesWrapperTest, getWindowCharExceptionTest) {
  const int numRows = 10;
  const int numCols = 10;
  const int rowStart = 3;
  const int colStart = 7;
  ...
  EXPECT_THROW(win.getWindowChar(-1, 3), std::out_of_range);
  EXPECT_THROW(win.getWindowChar(4, -2), std::out_of_range);
  EXPECT_THROW(win.getWindowChar(10, 3), std::out_of_range);
  EXPECT_THROW(win.getWindowChar(4, 10), std::out_of_range);
  EXPECT_THROW(win.getWindowChar(10, 10), std::out_of_range);
  EXPECT_THROW(win.getWindowChar(2, 13), std::out_of_range);
  EXPECT_THROW(win.getWindowChar(5, 30), std::out_of_range);
}

TEST_F(CursesWrapperTest, getCharInputExceptionTest) {
  const int numRows = 10;
  const int numCols = 10;
  const int rowStart = 3;
  const int colStart = 7;
  ...
  EXPECT_THROW(win.getCharInput(-1, 3), std::out_of_range);
  EXPECT_THROW(win.getCharInput(4, -2), std::out_of_range);
  EXPECT_THROW(win.getCharInput(10, 3), std::out_of_range);
  EXPECT_THROW(win.getCharInput(4, 10), std::out_of_range);
  EXPECT_THROW(win.getCharInput(10, 10), std::out_of_range);
  EXPECT_THROW(win.getCharInput(2, 13), std::out_of_range);
  EXPECT_THROW(win.getCharInput(5, 30), std::out_of_range);
}

TEST_F(CursesWrapperTest, getStringInputExceptionTest) {
  const int numRows = 10;
  const int numCols = 10;
  const int rowStart = 3;
  const int colStart = 7;
  ...
  EXPECT_THROW(win.getStringInput(-1, 3), std::out_of_range);
  EXPECT_THROW(win.getStringInput(4, -2), std::out_of_range);
  EXPECT_THROW(win.getStringInput(10, 3), std::out_of_range);
  EXPECT_THROW(win.getStringInput(4, 10), std::out_of_range);
  EXPECT_THROW(win.getStringInput(10, 10), std::out_of_range);
  EXPECT_THROW(win.getStringInput(2, 13), std::out_of_range);
  EXPECT_THROW(win.getStringInput(5, 30), std::out_of_range);
}

TEST_F(CursesWrapperTest, addCharacterExceptionTest) {
  const int numRows = 10;
  const int numCols = 10;
  const int rowStart = 3;
  const int colStart = 7;
  ...
  EXPECT_THROW(win.addCharacter(-1, 3, 'a'), std::out_of_range);
  EXPECT_THROW(win.addCharacter(4, -2, 'b'), std::out_of_range);
  EXPECT_THROW(win.addCharacter(10, 3, 'c'), std::out_of_range);
  EXPECT_THROW(win.addCharacter(4, 10, 'd'), std::out_of_range);
  EXPECT_THROW(win.addCharacter(10, 10, 'e'), std::out_of_range);
  EXPECT_THROW(win.addCharacter(2, 13, 'f'), std::out_of_range);
  EXPECT_THROW(win.addCharacter(5, 30, 'g'), std::out_of_range);
}

TEST_F(CursesWrapperTest, addStringExceptionTest) {
  const int numRows = 10;
  const int numCols = 10;
  const int rowStart = 3;
  const int colStart = 7;
  ...
  EXPECT_THROW(win.addString(-1, 3, "a"), std::out_of_range);
  EXPECT_THROW(win.addString(4, -2, "b"), std::out_of_range);
  EXPECT_THROW(win.addString(10, 3, "c"), std::out_of_range);
  EXPECT_THROW(win.addString(4, 10, "d"), std::out_of_range);
  EXPECT_THROW(win.addString(10, 10, "e"), std::out_of_range);
  EXPECT_THROW(win.addString(2, 13, "f"), std::out_of_range);
  EXPECT_THROW(win.addString(5, 30, "g"), std::out_of_range);
  EXPECT_NO_THROW(win.addString(9, 0, "This is a string that is really long"));
}

TEST_F(CursesWrapperTest, BracketWriteExceptionTest) {
  const int numRows = 10;
  const int numCols = 10;
  const int rowStart = 3;
  const int colStart = 7;
  ...
  EXPECT_THROW(win[-1][3] = 'a', std::out_of_range);
  EXPECT_THROW(win[4][-2] = 'b', std::out_of_range);
  EXPECT_THROW(win[10][3] = 'c', std::out_of_range);
  EXPECT_THROW(win[4][10] = 'd', std::out_of_range);
  EXPECT_THROW(win[10][10] = 'e', std::out_of_range);
  EXPECT_THROW(win[2][13] = 'f', std::out_of_range);
  EXPECT_THROW(win[5][30] = 'g', std::out_of_range);
}

TEST_F(CursesWrapperTest, BracketReadExceptionTest) {
  const int numRows = 10;
  const int numCols = 10;
  const int rowStart = 3;
  const int colStart = 7;
  ...
  EXPECT_THROW(std::cout << win[-1][3], std::out_of_range);
  EXPECT_THROW(std::cout << win[4][-2], std::out_of_range);
  EXPECT_THROW(std::cout << win[10][3], std::out_of_range);
  EXPECT_THROW(std::cout << win[4][10], std::out_of_range);
  EXPECT_THROW(std::cout << win[10][10], std::out_of_range);
  EXPECT_THROW(std::cout << win[2][13], std::out_of_range);
  EXPECT_THROW(std::cout << win[5][30], std::out_of_range);
}
